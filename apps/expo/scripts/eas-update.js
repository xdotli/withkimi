...
if (!config) {
  throw new Error('Failed to import Expo config. Are you in your app directory?')
}
...
function parseUpdateInfo(infoString = '') {
  ...
  if (!infoString.includes(androidPrefix) && !infoString.includes(iosPrefix)) {
    return {}
  }
...
const run = async () => {
  ...
  const profile = Object.values(easConfig.build).find(
    (build) => build.env.STAGE === process.env.STAGE
  ).channel
  ...
  const getRemoteVersions = async () => {
    try {
      ...
      let output = []
      console.log()
      console.log(chalk.green('[eas-update-sentry] Running the following command:'))
  ...
  try {
    ...
    stdout.on('data', (data) => {
      ...
      const { iosUpdateId: _iosUpdateId, androidUpdateId: _androidUpdateId } =
        parseUpdateInfo(stringData)
    ...
    const getBundles = () => {
      ...
      const androidBundle = bundles.find((s) => s.startsWith('android-') && s.endsWith('.js'))
      const androidMap = bundles.find((s) => s.startsWith('android-') && s.endsWith('.map'))
    ...
    const uploadSourceMap = async ({ updateId, buildNumber, bundleIdentifier, platform }) => {
      ...
      const args = [
        ...
        updateId,
        '--rewrite',
        platform === 'ios' ? 'dist/bundles/main.jsbundle' : 'dist/bundles/index.android.bundle',
        platform === 'ios' ? `dist/bundles/${iosMap}` : `dist/bundles/${androidMap}`,
      ]
      ...
      const result = spawnAsync(sentryCliPath, args, {
        env: {
          ...
          SENTRY_URL: sentryConfig?.url || process.env.SENTRY_URL || 'https://sentry.io/',
        },
      })
    ...
    const uploadIosSourceMap = async (buildNumber) => {
      console.log(
        `[eas-update-sentry] iOS: ${JSON.stringify({ iosUpdateId, iosBundle, iosMap }, null, 4)}`
      )
    ...
    const uploadAndroidSourceMap = async (buildNumber) => {
      console.log(
        `[eas-update-sentry] Android: ${JSON.stringify(
          { iosUpdateId, iosBundle, iosMap },
          null,
          4
        )}`
      )
      ...
      if (androidUpdateId && androidBundle && androidMap) {
        console.log()
        console.log(chalk.green('[eas-update-sentry] Updating Android Bundle File...'))

        fs.renameSync(`dist/bundles/${androidBundle}`, 'dist/bundles/index.android.bundle')
        const androidConfig = {
